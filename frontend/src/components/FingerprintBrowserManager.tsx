...
export const FingerprintBrowserManager: React.FC = () => {
  const [accounts, setAccounts] = useState<FingerprintAccount[]>([]);
  ...
  // 模拟指纹浏览器账号数据
  const mockAccounts: FingerprintAccount[] = [
    {
      id: 'fp_001',
      name: 'John Smith',
      phone: '+1-555-0123',
      ...
      isSelected: false,
      isVisible: true,
      streamUrl: 'ws://localhost:8080/stream/fp_001'
    },
    {
      id: 'fp_002',
      name: 'Maria Garcia',
      ...
      isSelected: false,
      isVisible: true,
      streamUrl: 'ws://localhost:8080/stream/fp_002'
    },
    {
      id: 'fp_003',
      name: 'Li Wei',
      ...
      isSelected: false,
      isVisible: true,
      streamUrl: 'ws://localhost:8080/stream/fp_003'
    }
  ];
  ...
  const renderRealTimeStreams = () => (
    <div style={{ 
      ...
      {streamsVisible && (
        <Row gutter={[16, 16]}>
          {accounts.filter(acc => acc.isVisible).map(account => (
            <Col 
              key={account.id} 
